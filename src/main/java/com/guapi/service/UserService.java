...
//        return userMapper.selectById(userId);
    ...
    public Map<String ,Object> register(User user){
        ...
        //验证账号
        User judgeUser = userMapper.selectByName(user.getUsername());
        if (judgeUser != null) {
            map.put("usernameMsg","该用户已经存在");
            return map;
        }
        //验证邮箱
        judgeUser = userMapper.selectByEmail(user.getEmail());
        if (judgeUser != null) {
    ...
    public Map<String ,Object> login(String username,String password,int expiredSeconds){
        ...
        System.out.println("从loginTicket中取出userid------"+loginTicket.getUserId());
//        loginTicketMapper.insertLoginTicket(loginTicket);
        String redisKey = RedisUtil.getTicketKey(loginTicket.getTicket());
        //redis会把对象序列化成一个json字符格式串保存
        redisTemplate.opsForValue().set(redisKey,loginTicket);

//        System.out.println("将用户信息加入到了loginTicket中");
